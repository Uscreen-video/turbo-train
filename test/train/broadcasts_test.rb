...
class BroadcastsTest < ActiveSupport::TestCase
  include Turbo::Train::TestHelper
  ...
  test 'broadcasting render' do
    ...
    assert_broadcast_on 'messages', turbo_stream_action_tag('replace', target: 'message_1', template: 'Goodbye!') do
      r = Turbo::Train.broadcast_render_to('messages', partial: 'messages/message', locals: { message: })
  ...
  test 'broadcast_render_later_to' do
    ...
    assert_broadcast_on 'messages', turbo_stream_action_tag('replace', target: 'message_1', template: 'Goodbye!') do
      perform_enqueued_jobs do
        Turbo::Train.broadcast_render_later_to('messages', partial: 'messages/message', locals: { message: })
      end
    end
  end
end
