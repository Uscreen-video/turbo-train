...
class BroadcastingTest < ApplicationSystemTestCase
  test 'Turbo::Train broadcasts Turbo Streams' do
    ...
    assert_no_javascript_errors do
      ...
      Turbo::Train.broadcast_append_to('messages', target: list_target, partial: 'messages/message',
                                                   locals: { message: Message.create!(user: user_bob, text: 'Hi John!') })
      sleep(2)
      assert_text 'Hi John!'
      ...
      Turbo::Train.broadcast_update_to('messages', target: ENV['TEST_MSG_ID'] + Message.last.id.to_s,
                                                   partial: 'messages/message', locals: { message: Message.last })
      sleep(2)
      assert_text 'Hi John! How are you? [edited]'

      Message.last.update!(text: "Hi John! How's it going? [edited]")
      Turbo::Train.broadcast_replace_to('messages', target: ENV['TEST_MSG_ID'] + Message.last.id.to_s,
                                                    partial: 'messages/message', locals: { message: Message.last })
      sleep(2)
      ...
      Turbo::Train.broadcast_action_to('messages', action: :append, target: list_target,
                                                   html: "<p id='message'>2345</p>")
      sleep(2)
      assert_text '2345'
    end
  end
